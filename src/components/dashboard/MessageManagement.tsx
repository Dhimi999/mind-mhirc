    ...
    try {
      ...
      const messagesWithSenders = await Promise.all(
        data.map(async (message) => {
          ...
          return { ...message, read_by: convertedReadBy, sender: senderData } as ChatMessage;
        })
      );
      ...
      for (const msg of unreadMessages) {
        ...
        await supabase.from("chat_messages").update({ read_by: updatedReadBy }).eq("id", msg.id);
      }
    } catch (error) {
        ...
        async (payload) => {
          ...
          if (!senderError && senderData) {
            ...
            if (newMessage.sender_id !== user.id) {
              ...
              await supabase.from("chat_messages").update({ read_by: updatedReadBy }).eq("id", newMessage.id);
            }
          }
        }
      )
                      ...
                      <Dialog>
                        ...
                        <DialogContent>
                          <DialogHeader>
                            ...
                            <DialogDescription>Pilih pengguna untuk memulai obrolan baru.</DialogDescription>
                          </DialogHeader>
                                ...
                                filteredUsers.map((user) => (
                                  ...
                                  >
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <p className="font-medium truncate">{user.full_name || "Pengguna tanpa nama"}</p>
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center">
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="font-medium truncate">{otherParticipant?.full_name || "Pengguna"}</p>
                              ...
                              <div key={message.id} className={`flex ${isCurrentUser ? "justify-end" : "justify-start"}`}>
                                <div className={`max-w-[80%] ${isCurrentUser ? "bg-primary text-white" : "bg-muted"} px-4 py-2 rounded-lg`}>
                                  <div className="text-sm mb-1 flex justify-between items-center">
                                    {!isCurrentUser && (
                                      <span className="font-medium mr-2">{message.sender?.full_name || "Pengguna"}</span>
                                    )}
        ...
        <TabsContent value="broadcasts" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Pesan penting dan pengumuman dari admin Mind MHIRC.</CardDescription>
            </CardHeader>
            <CardContent>
