...
const MessageManagement: React.FC = () => {
  ...
  useEffect(() => {
    if (userSearch.trim() === "") {
      ...
      setFilteredUsers(availableUsers.filter(user => user.full_name?.toLowerCase().includes(lowercaseSearch) || user.account_type?.toLowerCase().includes(lowercaseSearch)));
    }
  ...
  useEffect(() => {
    ...
    const fetchChatRooms = async () => {
      ...
      try {
        ...
        } = await supabase.from("chat_participants").select("chat_room_id").eq("user_id", user.id);
        ...
        const {
          data: roomsData,
          error: roomsError
        } = await supabase.from("chat_rooms").select("*").in("id", roomIds).order("updated_at", {
          ascending: false
        });
        ...
        const roomsWithParticipants = await Promise.all(roomsData.map(async room => {
          const {
            ...
            error: roomParticipantsError
          } = await supabase.from("chat_participants").select("user_id").eq("chat_room_id", room.id);
          if (roomParticipantsError) throw roomParticipantsError;
          const userIds = roomParticipants.map(p => p.user_id);
          const {
            data: profilesData,
            error: profilesError
          } = await supabase.from("profiles").select("id, full_name, avatar_url, is_admin, account_type").in("id", userIds);
    ...
    fetchChatRooms();
    const roomsSubscription = supabase.channel("chat_rooms_changes").on("postgres_changes", {
      event: "*",
      schema: "public",
      table: "chat_rooms"
    }, () => fetchChatRooms()).subscribe();
    return () => {
    ...
    const fetchAvailableUsers = async () => {
      try {
        const {
          data,
          error
        } = await supabase.from("profiles").select("id, full_name, avatar_url, is_admin, account_type").order("full_name");
  ...
  const refreshMessages = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from("chat_messages").select("*").eq("chat_room_id", selectedRoomId).order("created_at");
      if (error) throw error;
      const messagesWithSenders = await Promise.all(data.map(async message => {
        const {
          data: senderData,
          error: senderError
        } = await supabase.from("profiles").select("id, full_name, avatar_url, is_admin, account_type").eq("id", message.sender_id).single();
      ...
      const unreadMessages = messagesWithSenders.filter(msg => msg.sender_id !== user.id && !msg.read_by.includes(user.id));
  ...
  useEffect(() => {
    refreshMessages();
    const messagesSubscription = supabase.channel("chat_messages_changes").on("postgres_changes", {
      event: "INSERT",
      schema: "public",
      table: "chat_messages",
      ...
      const {
        data: senderData,
        error: senderError
      } = await supabase.from("profiles").select("id, full_name, avatar_url, is_admin, account_type").eq("id", newMessage.sender_id).single();
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (broadcastsData && broadcastsData.length > 0) {
          filteredBroadcasts = broadcastsData.filter(broadcast => {
            ...
            const broadcastRecipients = Array.isArray(broadcast.recipients) ? broadcast.recipients : typeof broadcast.recipients === "string" ? [broadcast.recipients] : [];
            return broadcastRecipients.includes("all") || broadcastRecipients.includes(userAccountType) || user.is_admin && broadcastRecipients.includes("professional");
          }).map(broadcast => {
    ...
    fetchBroadcasts();
    const broadcastsSubscription = supabase.channel("broadcasts_changes").on("postgres_changes", {
      event: "INSERT",
      schema: "public",
      table: "broadcasts"
    }, () => fetchBroadcasts()).subscribe();
    const recipientsSubscription = supabase.channel("broadcast_recipients_changes").on("postgres_changes", {
      event: "*",
      schema: "public",
  ...
  const handleCreateChatRoom = async () => {
    ...
    try {
      ...
      const {
        data: selectedUser,
        error: userError
      } = await supabase.from("profiles").select("id, full_name, avatar_url, is_admin, account_type").eq("id", selectedUserId).single();
      if (userError) throw userError;
      const {
        data: currentUser,
        error: currentUserError
      } = await supabase.from("profiles").select("id, full_name, avatar_url, is_admin, account_type").eq("id", user.id).single();
  ...
  const handleDeleteChatRoom = async () => {
    ...
    try {
      ...
      const {
        error: messagesError
      } = await supabase.from("chat_messages").delete().eq("chat_room_id", selectedRoomId);
      if (messagesError) throw messagesError;
      const {
        error: participantsError
      } = await supabase.from("chat_participants").delete().eq("chat_room_id", selectedRoomId);
      if (participantsError) throw participantsError;
      const {
        error: roomError
      } = await supabase.from("chat_rooms").delete().eq("id", selectedRoomId);
      if (roomError) throw roomError;
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-2 mb-4">
          ...
          <TabsTrigger value="broadcasts" className="flex items-center">
            ...
            Pesan Siaran
            {unreadBroadcastsCount > 0 && <span className="ml-2 bg-primary text-white text-xs py-0.5 px-1.5 rounded-full">
            ...
            <div className="lg:col-span-1">
              <Card className="h-full">
                        ...
                        <DialogContent>
                          ...
                          <div className="py-4 space-y-4">
                            <div className="flex items-center space-x-2">
                              ...
                              <Input placeholder="Cari pengguna..." value={userSearch} onChange={e => setUserSearch(e.target.value)} className="flex-1" />
                              {userSearch && <Button variant="ghost" size="icon" onClick={() => setUserSearch("")}>
                                  <X className="h-4 w-4" />
                                </Button>}
                            </div>
                            <div className="max-h-60 overflow-y-auto border rounded-md p-1">
                              {filteredUsers.length === 0 ? <div className="text-center py-4 text-muted-foreground">
                                  {userSearch ? "Tidak ada pengguna yang ditemukan" : "Tidak ada pengguna yang tersedia"}
                                </div> : filteredUsers.map(user => <div key={user.id} className={`flex items-center space-x-3 p-2 rounded-md cursor-pointer transition-colors ${selectedUserId === user.id ? "bg-muted" : "hover:bg-muted/50"}`} onClick={() => setSelectedUserId(user.id)}>
                                    {user.avatar_url ? <img src={user.avatar_url} alt={user.full_name || "User"} className="h-10 w-10 rounded-full object-cover" /> : <div className="h-10 w-10 rounded-full bg-muted flex items-center justify-center">
                ...
                <CardContent className="h-[500px] overflow-y-auto">
                  {isFetchingRooms ? <div className="text-center py-8">
                      <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary mx-auto mb-2"></div>
                      <p className="text-sm text-muted-foreground">
                        Memuat ruang obrolan...
                      </p>
                    </div> : chatRooms.length === 0 ? <div className="flex flex-col items-center justify-center h-full text-center p-4">
                    ...
                    return <div key={room.id} className={`p-3 rounded-lg cursor-pointer hover:bg-muted transition-colors relative ${selectedRoomId === room.id ? "bg-muted border border-primary/20" : "bg-card"}`} onClick={() => setSelectedRoomId(room.id)}>
                            <div className="flex items-center">
                              <div className="flex-shrink-0 mr-3">
                                {otherParticipant?.avatar_url ? <img src={otherParticipant.avatar_url} alt={otherParticipant.full_name || "User"} className="h-10 w-10 rounded-full object-cover" /> : <div className="h-10 w-10 rounded-full bg-muted flex items-center justify-center">
                                    <User className="h-6 w-6 text-muted-foreground" />
                                  </div>}
                              </div>
                              <div className="flex-1 min-w-0">
                              ...
                              {room.last_message_at && <div className="text-xs text-muted-foreground ml-2">
                                  {format(new Date(room.last_message_at), "HH:mm")}
                                </div>}
                              {canDelete && selectedRoomId === room.id && <AlertDialog open={showConfirmDelete} onOpenChange={setShowConfirmDelete}>
              ...
              <Card className="h-full flex flex-col">
                {selectedRoomId ? <>
                    <CardHeader className="pb-3 border-b flex-shrink-0">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center">
                          ...
                          return otherParticipant?.avatar_url ? <img src={otherParticipant.avatar_url} alt={otherParticipant.full_name || "User"} className="h-10 w-10 rounded-full object-cover" /> : <div className="h-10 w-10 rounded-full bg-muted flex items-center justify-center">
                                  <User className="h-6 w-6 text-muted-foreground" />
                                </div>;
                        })()}
                          </div>
                          <div>
                            ...
                            <CardDescription>
                              {chatRooms.find(r => r.id === selectedRoomId)?.participants.find(p => p.id !== user?.id)?.account_type === "professional" ? "Profesional" : chatRooms.find(r => r.id === selectedRoomId)?.participants.find(p => p.id !== user?.id)?.is_admin ? "Admin" : "Pengguna Umum"}
                            </CardDescription>
                          </div>
                        </div>
                        <div className="flex items-center space-x-1">
                          ...
                          {isRoomCreator(chatRooms.find(r => r.id === selectedRoomId)!) && <AlertDialog>
                              <AlertDialogTrigger asChild>
                    ...
                    <CardContent className="h-[380px] overflow-y-auto p-4">
                      {isFetchingMessages ? <div className="text-center py-8">
                          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary mx-auto mb-2"></div>
                          <p className="text-sm text-muted-foreground">
                            Memuat pesan...
                          </p>
                      ...
                      return <div key={message.id} className={`flex ${isCurrentUser ? "justify-end" : "justify-start"}`}>
                                <div className={`max-w-[80%] ${isCurrentUser ? "bg-primary text-white" : "bg-muted"} px-4 py-2 rounded-lg`}>
                                  <div className="text-sm mb-1 flex justify-between items-center">
                                    {!isCurrentUser && <span className="font-medium mr-2">
                    ...
                    <CardFooter className="border-t p-3">
                      <form className="flex w-full items-center space-x-2" onSubmit={e => {
                    e.preventDefault();
                    handleSendMessage();
                  }}>
                        <Textarea value={newMessage} onChange={e => setNewMessage(e.target.value)} placeholder="Ketik pesan..." className="min-h-[60px] flex-1 resize-none" onKeyDown={e => {
        ...
        <TabsContent value="broadcasts" className="space-y-4">
          <Card>
            ...
            <CardContent>
              {isFetchingBroadcasts ? <div className="text-center py-8">
                  <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary mx-auto mb-2"></div>
                  <p className="text-sm text-muted-foreground">
                    Memuat pesan siaran...
                  </p>
                </div> : broadcasts.length === 0 ? <div className="text-center py-8">
                ...
                return <div key={broadcast.id} className={`border rounded-lg p-4 ${broadcast.is_read ? "bg-card" : "bg-muted border-primary/20"} ${broadcast.priority === "urgent" ? "border-red-500" : ""}`}>
                        ...
                        <div className="flex justify-between items-center text-sm text-muted-foreground">
                          ...
                          {broadcast.is_read ? <div className="flex items-center text-green-600">
                              <CheckCircle className="h-4 w-4 mr-1" />
                              <span>Telah dibaca</span>
                            </div> : <Button variant="outline" size="sm" onClick={() => handleMarkBroadcastRead(broadcast.id)}>
                              Tandai Telah Dibaca
                            </Button>}
                        </div>
                      </div>;
