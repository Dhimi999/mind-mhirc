...
export const HibridaSessionCard: React.FC<SessionCardProps> = ({
  session,
  title,
  status,
  submissionCount = 0,
  ...
  return (
    <Card
      className={`group transition-all ${
        status === "available"
          ? "hover:shadow-lg border-teal-200 bg-teal-50/50"
      ...
      <CardContent className="p-6">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div
              className={`flex-shrink-0 w-12 h-12 sm:w-16 sm:h-16 rounded-full flex items-center justify-center text-white font-bold text-base sm:text-lg ${
                status === "available" ? "bg-teal-600" : "bg-muted"
              }`}
            >
              {session}
            </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch gap-3">
          ...
          <div className="flex sm:w-[180px] justify-end items-center">
            <Button
              className={status === "available" ? "bg-teal-600 hover:bg-teal-700" : ""}
