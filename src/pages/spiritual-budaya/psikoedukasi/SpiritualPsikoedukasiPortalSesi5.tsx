...
const ASSIGNMENT_KEY = "spiritualPsikoEduSession5Assignment";

interface AssignmentData {
  situasi: string;
  dampakStigma: string;
  upayaMengurangiStigma: string;
  hasil: string;
  refleksi: string;
  jurnal: string;
  submitted?: boolean;
...
const SpiritualPsikoedukasiPortalSesi5: React.FC = () => {
  const sessionNumber = 5;
  const title = "Pengurangan Stigma Bagi Mahasiswa";
  const { user } = useAuth();
  ...
  const assignmentValid = useMemo(() => [assignment.situasi, assignment.dampakStigma, assignment.upayaMengurangiStigma, assignment.hasil, assignment.refleksi, assignment.jurnal].every(f=> f.trim()), [assignment]);
  ...
  const assignmentFillPercent = useMemo(()=> { const fields=[assignment.situasi, assignment.dampakStigma, assignment.upayaMengurangiStigma, assignment.hasil, assignment.refleksi, assignment.jurnal]; const filled=fields.filter(x=> x.trim()).length; return Math.round((filled/fields.length)*100); }, [assignment]);
  ...
  const renderGuide = () => { if (!progress.meetingDone) return <div className="text-sm text-muted-foreground">Panduan akan muncul setelah Anda menandai Pertemuan Daring selesai.</div>; return (
    <div className="space-y-4 text-sm leading-relaxed">
      <div className="rounded-lg border-l-4 border-indigo-600 bg-gradient-to-r from-indigo-50 to-white p-4">
        <p className="font-semibold text-indigo-800 mb-1">Fokus Sesi 5 â€” Pengurangan Stigma</p>
  <p>Pengurangan stigma berfokus pada menurunkan stigma terkait kesehatan mental dan bunuh diri melalui diskusi dan edukasi. Tujuannya mendorong keterbukaan, empati, dan keberanian untuk mencari bantuan.</p>
      </div>
      <ul className="list-disc pl-5 space-y-1 text-muted-foreground">
  <li>Diskusikan contoh bahasa dan perilaku yang menstigma vs. yang inklusif.</li>
  <li>Identifikasi dampak stigma pada pikiran, emosi, dan perilaku Anda.</li>
  <li>Tuliskan 2â€“3 langkah konkret untuk mengurangi stigma di lingkungan Anda.</li>
      </ul>
  <div className="border rounded p-3 bg-indigo-50 text-xs text-indigo-800">Catatan: Gunakan bahasa inklusif dan empatik â€” ini langkah sederhana namun efektif untuk menurunkan stigma.</div>
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Helmet>
        <title>Sesi 5 - {title} | Psikoedukasi</title>
        <meta name="description" content={`Portal psikoedukasi sesi 5: ${title}`} />
        <link rel="canonical" href="https://mind-mhirc.my.id/spiritual-budaya/psikoedukasi/sesi/5" />
      </Helmet>
      <Navbar />
        ...
        <section className="relative overflow-hidden rounded">
          ...
          <div className="relative container mx-auto px-6 py-14">
            <div className="max-w-5xl mx-auto">
              ...
              <h1 className="text-3xl md:text-5xl font-bold mb-3 text-white drop-shadow-sm">Sesi 5: {title}</h1>
              <p className="text-amber-100 max-w-3xl">Pengurangan stigma adalah sesi untuk mengurangi stigma terkait kesehatan mental dan bunuh diri. Melalui diskusi dan edukasi, mahasiswa diharapkan lebih terbuka dalam berbicara tentang kesehatan mental dan tidak merasa malu untuk mencari bantuan.</p>
            </div>
          </div>
        </section>
          ...
          <div className="container mx-auto px-6 max-w-6xl">
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
              <div className="lg:col-span-1 space-y-6">
                ...
                <div className="rounded-xl border border-amber-100 p-4 text-xs leading-relaxed bg-white/60 backdrop-blur"><p className="font-semibold mb-1 text-amber-700">Tips</p><p>Gunakan bahasa yang netral dan empatik saat membahas isu sensitif.</p></div>
              </div>
              <div className="lg:col-span-3 space-y-8">
                    ...
                    <div className="bg-white rounded-sm">
                      <CardHeader className="bg-gradient-to-r from-amber-50 to-orange-50">
                        <div className="flex items-center gap-3">
                          <div className="p-2 bg-amber-600 rounded-lg"><svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg></div>
                          <div><CardTitle className="text-amber-800">Informasi Pertemuan Daring</CardTitle><CardDescription>Jadwal dan link pertemuan sesi ini</CardDescription></div>
                        </div>
                      </CardHeader>
                      <CardContent className="p-6">
                        {loading ? (
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                            {(['A','B','C'] as const).map(k => (
                              <div key={k} className="rounded border p-3 bg-muted/30">
                                <div className="flex items-center justify-between mb-1"><span className="text-xs text-muted-foreground">Grup</span><span className="px-2 py-0.5 text-[11px] rounded-full bg-amber-100 text-amber-800 border border-amber-200">{k}</span></div>
                          ...
                          <>
                            <div className="mb-2 flex items-center gap-2">{meeting?.has_group_schedules && (groupAssignment === 'A' || groupAssignment === 'B' || groupAssignment === 'C') && (<span className="px-2 py-0.5 text-[11px] rounded-full bg-purple-100 text-purple-800 border border-purple-200">Grup {groupAssignment}</span>)}</div>
                        ...
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                          ...
                          <div className="sm:ml-auto"><Badge className={progress.meetingDone ? 'bg-green-600 text-white' : 'bg-amber-200 text-amber-900'}>{progress.meetingDone ? '✓ Sudah selesai' : '⏳ Belum selesai'}</Badge></div>
                        </div>
                      </CardContent>
                    </div>
                  </div>
                ...
                <Card className="border-amber-100 shadow-md"><CardHeader><CardTitle>Penugasan</CardTitle><CardDescription>Eksplorasi dampak stigma & langkah pengurangan</CardDescription></CardHeader><CardContent>{!progress.guideDone && !progress.assignmentDone && (<div className="mb-4 p-3 rounded border border-amber-300 bg-amber-50 text-amber-900 text-sm">Penugasan terkunci. Selesaikan Panduan Penugasan untuk membuka.</div>)}<div className={(!progress.guideDone && !progress.assignmentDone)?'pointer-events-none opacity-60 select-none':''}><div className="space-y-8"><div><h4 className="font-semibold mb-3">Identitas Peserta</h4><div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-2"><div><label className="block text-xs font-medium mb-1">Nama</label><input className="w-full border rounded p-2 text-sm bg-gray-50" value={user?.full_name||'-'} disabled /></div><div><label className="block text-xs font-medium mb-1">Hari</label><input className="w-full border rounded p-2 text-sm bg-gray-50" value={new Date().toLocaleDateString('id-ID',{ weekday:'long'})} disabled /></div><div><label className="block text-xs font-medium mb-1">Tanggal</label><input className="w-full border rounded p-2 text-sm" value={new Date().toLocaleDateString('id-ID')} disabled /></div></div></div><div><h4 className="font-semibold mb-3">1. Situasi</h4><textarea rows={3} className="w-full rounded border p-2 text-sm" value={assignment.situasi} onChange={e=> setAssignment(p=> ({ ...p, situasi: e.target.value }))} disabled={progress.assignmentDone} /></div><div><h4 className="font-semibold mb-3">2. Dampak Stigma</h4><textarea rows={3} className="w-full rounded border p-2 text-sm" value={assignment.dampakStigma} onChange={e=> setAssignment(p=> ({ ...p, dampakStigma: e.target.value }))} disabled={progress.assignmentDone} /></div><div><h4 className="font-semibold mb-3">3. Upaya Mengurangi Stigma</h4><textarea rows={3} className="w-full rounded border p-2 text-sm" value={assignment.upayaMengurangiStigma} onChange={e=> setAssignment(p=> ({ ...p, upayaMengurangiStigma: e.target.value }))} disabled={progress.assignmentDone} /></div><div><h4 className="font-semibold mb-3">4. Hasil</h4><textarea rows={3} className="w-full rounded border p-2 text-sm" value={assignment.hasil} onChange={e=> setAssignment(p=> ({ ...p, hasil: e.target.value }))} disabled={progress.assignmentDone} /></div><div><h4 className="font-semibold mb-3">5. Refleksi</h4><textarea rows={3} className="w-full rounded border p-2 text-sm" value={assignment.refleksi} onChange={e=> setAssignment(p=> ({ ...p, refleksi: e.target.value }))} disabled={progress.assignmentDone} /></div><div className="flex items-center gap-3 pt-2 border-t"><Button className="bg-amber-600 hover:bg-amber-700" disabled={!assignmentValid || progress.assignmentDone} onClick={handleSubmitAssignment}>{progress.assignmentDone? 'Terkirim':'Kirim & Tandai Selesai'}</Button><Badge className={progress.assignmentDone?'bg-green-600 text-white':'bg-amber-200 text-amber-900'}>{progress.assignmentDone? 'Sudah selesai':'Belum selesai'}</Badge>{autoSavedAt && !progress.assignmentDone && <span className="text-xs text-muted-foreground">Draft tersimpan: {autoSavedAt}</span>}</div><div className="text-[11px] text-muted-foreground">Progress penugasan: {assignmentFillPercent}%</div></div></div></CardContent></Card>
...
export default SpiritualPsikoedukasiPortalSesi5;
