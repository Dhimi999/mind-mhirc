...
const ASSIGNMENT_KEY = "spiritualPsikoEduSession2Assignment";
...
// Meeting schedule now loaded dynamically via Supabase

const SpiritualPsikoedukasiPortalSesi2: React.FC = () => {
  const sessionNumber = 2;
  const title = "Mengenali Tanda-Tanda Dini Risiko Bunuh Diri Bagi Mahasiswa";
  const { user } = useAuth();

  const { progress, meetingSchedule: schedule, markMeetingDone, submitAssignment, loadAssignment, autoSaveAssignment, groupAssignment, isSuperAdmin, allGroupSchedules, loading } = useSpiritualPsikoedukasiSession(sessionNumber, user?.id);
  ...
  const updateProgress = (_p: Partial<SessionProgress>) => {}; // handled by hook

  const assignmentValid = useMemo(() => {
    if (!assignment.situasi.trim()) return false;
    if (!assignment.tanda.trim()) return false;
    if (!assignment.tindakan.trim()) return false;
  ...
  const overallPercent = useMemo(() => {
    let total = 0; if (progress.sessionOpened) total += 20; if (progress.meetingDone) total += 30; if (progress.assignmentDone) total += 30; if (progress.counselorResponse) total += 20; return total;
  }, [progress]);

  const assignmentFillPercent = useMemo(() => {
    const fields = [assignment.situasi, assignment.tanda, assignment.tindakan, assignment.hasil, assignment.refleksi, assignment.jurnal];
    const filled = fields.filter(f => f.trim()).length; return Math.round((filled / fields.length) * 100);
  }, [assignment]);

  const handleSubmitAssignment = useCallback(async () => {
    if (!assignmentValid || progress.assignmentDone) return;
    const ok = await submitAssignment(assignment);
    ...
    return (
      <div className="space-y-4 text-sm leading-relaxed">
        <div className="rounded-lg border-l-4 border-indigo-600 bg-gradient-to-r from-indigo-50 to-white p-4">
          <p className="font-semibold text-indigo-800 mb-1">Fokus Sesi 2 â€” Mengenali Tanda-Tanda Dini Risiko Bunuh Diri</p>
          <p>Mengenali tanda-tanda dan risiko bunuh diri adalah sesi yang mengajarkan mahasiswa untuk mengenali tanda-tanda awal dari ideasi bunuh diri, baik pada diri mereka sendiri maupun orang lain. Ini termasuk perubahan perilaku drastis seperti menarik diri dari pergaulan, peningkatan penggunaan zat terlarang, serta ekspresi verbal tentang keputusasaan. Dengan melakukan analisis ini, Anda dapat mengubah cara pandang menjadi lebih positif lagi.</p>
        </div>
        <ul className="list-disc pl-5 space-y-1 text-muted-foreground">
          <li>Mengidentifikasi situasi pemicu.</li>
          <li>Mencatat tanda-tanda dini (perubahan perilaku, emosi, kognisi).</li>
          <li>Menentukan tindakan preventif / support.</li>
          <li>Mengevaluasi hasil & melakukan refleksi.</li>
          <li>Mengelola jurnal harian terkait perkembangan emosi & pikiran.</li>
        </ul>
        <div className="border rounded p-3 bg-indigo-50 text-xs text-indigo-800">Catatan: Jika muncul intensi bunuh diri, segera hubungi layanan darurat atau profesional.</div>
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Helmet>
        <title>Sesi 2 - {title} | Psikoedukasi</title>
        <meta name="description" content={`Portal psikoedukasi sesi 2: ${title}`} />
        <link rel="canonical" href="https://mind-mhirc.my.id/spiritual-budaya/psikoedukasi/sesi/2" />
      </Helmet>
      <Navbar />
        ...
        <section className="relative overflow-hidden rounded">
          ...
          <div className="relative container mx-auto px-6 py-14">
            <div className="max-w-5xl mx-auto">
              ...
              <p className="text-indigo-100 max-w-2xl">Fokus pada identifikasi tanda-tanda dini & respons adaptif.</p>
            </div>
          </div>
        </section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
              ...
              <div className="lg:col-span-3 space-y-8">
                <Card className="border-indigo-100 shadow-sm overflow-hidden">
                  <div className="bg-gradient-to-r from-indigo-600 via-indigo-700 to-violet-600 p-1"><div className="bg-white rounded-sm">
                    <CardHeader className="bg-gradient-to-r from-indigo-50 to-violet-50">
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-indigo-600 rounded-lg"><svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg></div>
                        <div><CardTitle className="text-indigo-800">Pertemuan Daring</CardTitle><CardDescription>Sesi sinkron fasilitator</CardDescription></div>
                      </div>
                    </CardHeader>
                ...
                <Card className="border-indigo-100 shadow-sm">
                  <CardHeader><CardTitle>Panduan Sesi</CardTitle><CardDescription>Baca & centang konfirmasi sebelum penugasan.</CardDescription></CardHeader>
                  <CardContent>
                    {renderGuide()}
                ...
                <Card className="border-indigo-100 shadow-md">
                  ...
                  <CardContent>
                    {!hasReadGuide && !progress.assignmentDone && <div className="mb-4 p-3 rounded border border-indigo-300 bg-indigo-50 text-indigo-900 text-sm">Penugasan terkunci. Baca panduan lalu centang konfirmasi.</div>}
                    <div className={(!hasReadGuide && !progress.assignmentDone)?'pointer-events-none opacity-60 select-none':''}>
                      <div className="space-y-8">
                        ...
                        <div><h4 className="font-semibold mb-3">2. Tanda-tanda Dini Risiko Bunuh Diri</h4><textarea rows={3} className="w-full rounded border p-2 text-sm" value={assignment.tanda} onChange={e=> setAssignment(p=> ({ ...p, tanda: e.target.value }))} disabled={progress.assignmentDone} /></div>
                        <div><h4 className="font-semibold mb-3">3. Tindakan</h4><textarea rows={3} className="w-full rounded border p-2 text-sm" value={assignment.tindakan} onChange={e=> setAssignment(p=> ({ ...p, tindakan: e.target.value }))} disabled={progress.assignmentDone} /></div>
                        <div><h4 className="font-semibold mb-3">4. Hasil</h4><textarea rows={3} className="w-full rounded border p-2 text-sm" value={assignment.hasil} onChange={e=> setAssignment(p=> ({ ...p, hasil: e.target.value }))} disabled={progress.assignmentDone} /></div>
                        <div><h4 className="font-semibold mb-3">5. Refleksi</h4><textarea rows={3} className="w-full rounded border p-2 text-sm" value={assignment.refleksi} onChange={e=> setAssignment(p=> ({ ...p, refleksi: e.target.value }))} disabled={progress.assignmentDone} /></div>
                        <div><h4 className="font-semibold mb-3">6. Jurnal Harian</h4><textarea rows={5} className="w-full rounded border p-2 text-sm" value={assignment.jurnal} onChange={e=> setAssignment(p=> ({ ...p, jurnal: e.target.value }))} disabled={progress.assignmentDone} /></div>
...
export default SpiritualPsikoedukasiPortalSesi2;
