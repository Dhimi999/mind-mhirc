...
const HibridaPortalSesi: React.FC = () => {
  ...
  const sesi1Valid = useMemo(() => {
    ...
    // Activities: need >=3 with unique priorities
    const aktivitasEntries = Object.entries(a.aktivitas).filter(([, pr]) => pr !== undefined && pr !== null);
    if (aktivitasEntries.length < 3) return false;
    const prSet = new Set(aktivitasEntries.map(([, pr]) => pr));
    if (prSet.size !== aktivitasEntries.length) return false; // unique priorities
    // Safe environments >=3
    const lingkunganEntries = Object.entries(a.lingkungan).filter(([, pr]) => pr !== undefined && pr !== null);
    if (lingkunganEntries.length < 3) return false;
    const prSet2 = new Set(lingkunganEntries.map(([, pr]) => pr));
    if (prSet2.size !== lingkunganEntries.length) return false;
                  ...
                  {selected && (
                    <input
                      ...
                      onChange={e => {
                        ...
                        setAktivitasPriority(opt, val);
                      }}
                      disabled={progress.assignmentDone}
                      aria-label={`Prioritas untuk ${opt}`}
                    />
                  )}
                </div>
              );
                    ...
                    <CardContent className="p-6">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        ...
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          {schedule?.link ? (
                            <a className="text-indigo-700 underline font-medium" href={schedule.link} target="_blank" rel="noreferrer">Tersedia</a>
