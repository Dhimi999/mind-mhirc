...
const ASSIGNMENT_KEY_PREFIX = 'psikoEduSessionAssignment_';
...
const PsikoedukasiPortalSesi: React.FC = () => {
  ...
  const moduleNavigation = (
    <div className='flex flex-wrap gap-2 text-xs'>
      {Object.keys(psikoModules).map(n => {
        ...
        return (
          <button key={n} onClick={() => navigate(`/hibrida-cbt/psikoedukasi/sesi/${num}`)} className={`px-3 py-1 rounded-full border transition ${active ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white hover:bg-indigo-50 border-indigo-200 text-indigo-700'}`}>Modul {num}</button>
        );
      })}
    </div>
  );
  ...
  return (
    <div className='min-h-screen flex flex-col'>
      <Helmet>
        ...
        <link rel='canonical' href={`https://mind-mhirc.my.id/hibrida-cbt/psikoedukasi/sesi/${sessionNumber}`} />
      </Helmet>
      <Navbar />
          ...
          <div className='relative container mx-auto px-6 py-12'>
            <div className='max-w-5xl mx-auto text-white'>
              <div className='flex items-center justify-between mb-4'>
                <Link to='/hibrida-cbt/psikoedukasi' className='text-white/80 hover:underline text-sm'>← Kembali</Link>
                  ...
                  <CardContent>
                    <div className='space-y-4 text-sm leading-relaxed'>
                      ...
                      <ul className='list-disc pl-5 space-y-1 text-muted-foreground text-xs'>
                        <li>Konsep 1–2 paragraf singkat (placeholder).</li>
                      ...
                      <div className='flex items-center gap-3 pt-2'>
                        ...
                        <Badge className={progress.materialRead ? 'bg-green-600 text-white' : 'bg-amber-200 text-amber-900'}>{progress.materialRead ? '✓ Dibaca' : 'Belum Dibaca'}</Badge>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <CardContent>
                    {progress.counselorResponse ? (
                      ...
                      <div className='text-center py-8'>
                        ...
                        <p className='text-xs text-muted-foreground'>Umpan balik diberikan 1–2 hari kerja</p>
                      </div>
                    )}
                  </CardContent>
                </Card>
...
export default PsikoedukasiPortalSesi;
